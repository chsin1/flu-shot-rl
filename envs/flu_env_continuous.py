...
class FluVaccineEnvContinuous(gym.Env):
    ...
    def __init__(self, config: EnvConfig = BASELINE_CONFIG):
        ...
        self.action_space = spaces.Box(
            low=np.zeros(self.num_regions, dtype=np.float32),
    ...
    def step(self, action):
        ...
        orders_placed = np.array(action, dtype=np.float32)
        orders_placed = np.clip(orders_placed, 0.0, 1.0)
        orders_placed = orders_placed * 600.0
        orders_placed = np.round(orders_placed / 10.0) * 10.0
